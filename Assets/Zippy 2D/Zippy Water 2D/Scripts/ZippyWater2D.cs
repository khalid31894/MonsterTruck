...
public class ZippyWater2D : MonoBehaviour {
	...
	public void Splash(int index, float velocity) {
        ...
        if (velocity < -0f && splashSounds.Length > 0) PlaySound(velocity*2);
        if (velocity > 0f && exitSounds.Length > 0) PlaySound(velocity*2, true);
	...
	void PlaySound(float vel, bool exit = false) {
		if (!audioPlayed) {
			if (!exit) PlayClipAtPoint(splashSounds[Random.Range(0, splashSounds.Length)], transform.position, Mathf.Abs(vel * .5f));
			else PlayClipAtPoint(exitSounds[Random.Range(0, exitSounds.Length)], transform.position, Mathf.Abs(vel * .5f));
			audioPlayed = true;
