...
{
    public static EulerGet instance;
    // ===========================================
    // EULER CALC
    // ===========================================
    ...
    public void update_euler_calc()
    {
        ...
        if (Mathf.Abs(z_change) > 180)
        {
            ...
            if (z_change > 0)
            {
                // Add 360
                euler_z_360_pass += 1;
                Debug.Log("z chage in plus");
            }
            else
            {
                // Sub 360
                euler_z_360_pass -= 1;
                Debug.Log("z chage in nagetive");
            }
            // Recalc euler
            EulerZ = EulerZ_org + (360 * euler_z_360_pass);
        }
        ...
        //Debug.Log("Rotation = " + UnityEditor.TransformUtils.GetInspectorRotation(transform).z);
        if (last_euler_z >= 0)
        {
            eularSimple = last_euler_z % 360;
            if (eularSimple > 90 && eularSimple < 220)
            {
                GetComponent<Rigidbody2D>().AddTorque(-180);
                
            }
            else if (eularSimple > 220 && eularSimple < 340)
            {
                GetComponent<Rigidbody2D>().AddTorque(+180);
